--!strict
-- OnlyTwentyCharacters / AllYourBlox
...
local function CharacterOutlineBehavior(castPoints)
	...
	local cframe = CFrame.new(
		ZERO_VECTOR3,
		Vector3_new(Camera.CoordinateFrame.lookVector.X, 0, Camera.CoordinateFrame.lookVector.Z)
	)
			...
			if v0.unit:Dot(-v1) < v0.unit:Dot(vprime) then
				...
				if castPoint.Magnitude > 0 then
					...
					local hit, hitPoint, hitNormal =
						game.Workspace:FindPartOnRayWithIgnoreList(ray, { Character }, false, false)
