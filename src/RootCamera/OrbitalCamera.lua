...
local function SetAndBoundsCheckAzimuthValues()
	minAzimuthAbsoluteRad = math.rad(externalProperties["ReferenceAzimuth"])
		- math.abs(math.rad(externalProperties["CWAzimuthTravel"]))
	maxAzimuthAbsoluteRad = math.rad(externalProperties["ReferenceAzimuth"])
		+ math.abs(math.rad(externalProperties["CCWAzimuthTravel"]))
...
local function CreateOrbitalCamera()
		...
		if subjectPosition and player and camera then
			...
			if VREnabled and not self:IsInFirstPerson() then
				...
				if distToSubject > zoom or self.RotateInput.x ~= 0 then
					...
					vecToSubject = self:RotateCamera(vecToSubject.unit * XZ_VECTOR, Vector2.new(self.RotateInput.x, 0))
						* desiredDist
				...
				if useAzimuthLimits then
					...
					curAzimuthRad = (curAzimuthRad ~= 0)
							and (math.sign(curAzimuthRad) * (math.abs(curAzimuthRad) % TAU))
						or 0
				end
				...
				local cameraPosVector = curDistance
					* (CFrame.fromEulerAnglesYXZ(-curElevationRad, curAzimuthRad, 0) * UNIT_Z)
	...
	function module.doGamepadZoom(name, state, input)
		if
			input.UserInputType == module.activeGamepad
