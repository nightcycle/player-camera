...
local function CreateVRCamera()
	...
	local function onMovementUpdateEvent(updateType, arg1, arg2)
		...
		elseif
			updateType == "shortPath"
			or updateType == "pathFailure"
			or updateType == "offtrack" and not autoSnapsPaused
		then
			if isJumping then
	...
	spawn(function()
		...
		if not rootCamera then
			return
		end

		local cameraScript = rootCamera.Parent
		if not cameraScript then
			return
		end
	...
	local function onCurrentCameraChanged()
		...
		if workspace.CurrentCamera then
			onCameraSubjectChangedConn =
				workspace.CurrentCamera:GetPropertyChangedSignal("CameraSubject"):connect(onCameraSubjectChanged)
	...
	function module:Update()
		...
		if subjectPosition and player and camera then
			...
			camera.CFrame = CFrame.new(cameraFocusP - (HORZ_OFFSET * look))
