...
local function Pather(character, point)
	...
	function this:CheckOcclusion(point1, point2, character, torsoRadius)
		...
		local rightPart, _ =
			Utility.Raycast(Ray.new(point1 + rightVector, diffVector + rightVector), true, { character })
		...
		for i = 1, math_floor(diffVector.magnitude / studsBetweenSamples) do
			local downPart, _ = Utility.Raycast(
				Ray.new(point1 + directionVector * i * studsBetweenSamples, Vector3_new(0, -7, 0)),
				true,
				{ character }
			)
	...
	function this:SmoothPoints(pathToSmooth)
		...
		for i = #result - 1, 1, -1 do
			if i + 1 <= #result then
				...
				if
					lastPoint
					and Utility.FuzzyEquals(thisPoint.Y, lastPoint.Y)
					and Utility.FuzzyEquals(thisPoint.Y, nextPoint.Y)
				then
	...
	function this:CheckNeighboringCells(character)
		...
		if torso then
			...
			local roundedPos = Vector3_new(
				Utility.Round(torsoPos.X - 2, 4) + 2,
				Utility.Round(torsoPos.Y - 2, 4) + 2,
				Utility.Round(torsoPos.Z - 2, 4) + 2
			)
	...
	local function AllAxisInThreshhold(targetPt, otherPt, threshold)
		return math_abs(targetPt.X - otherPt.X) <= threshold
			and math_abs(targetPt.Y - otherPt.Y) <= threshold
			and math_abs(targetPt.Z - otherPt.Z) <= threshold
	end
	...
	function this:ComputePath()
		...
		if torso then
			...
			local success = pcall(function()
				...
				this.pathResult =
					PathfindingService:ComputeSmoothPathAsync(torso.CFrame.p - Vector3_new(0, 3, 0), point, 400)
				smoothed = true
			end)
			if not success then
				-- 3 is height from torso cframe to ground
				this.pathResult =
					PathfindingService:ComputeRawPathAsync(torso.CFrame.p - Vector3_new(0, 3, 0), point, 400)
				smoothed = false
			end
			...
			if
				this.pathResult.Status == Enum.PathStatus.ClosestNoPath
				and #this.pointList >= 1
				and pathFound == false
			then
				local otherPt = this.pointList[#this.pointList]
				if
					AllAxisInThreshhold(point, otherPt, 4)
					and (torso.CFrame.p - point).magnitude > (otherPt - point).magnitude
				then
					local pathResult = {}
					pathResult.Status = Enum.PathStatus.Success
					function pathResult:GetPointCoordinates()
			...
			if
				(
					this.pathResult.Status == Enum.PathStatus.FailStartNotEmpty
					or this.pathResult.Status == Enum.PathStatus.ClosestNoPath
				) and pathFound == false
			then
								...
								spawn(function()
									while yielding and this.Cancelled == false do
										if CameraModule then
											...
											if
												tick() - Utility.GetLastInput() > 2
												and theta > (workspace.CurrentCamera.FieldOfView / 2)
											then
												local rotatedCFrame = CameraModule:LookAtPreserveHeight(point)
												local finishedSignal, duration =
													CameraModule:TweenCameraLook(rotatedCFrame)
												--return
											end
										end
										wait(0.1)
...
local function ExploreWithRayCast(currentPoint, originDirection)
	...
	table.sort(testResults, function(a, b)
...
local function OnTap(tapPositions, goToPoint)
	...
	if #tapPositions == 1 or goToPoint then
		if camera then
			local unitRay =
				Utility.GetUnitRay(tapPositions[1].x, tapPositions[1].y, MyMouse.ViewSizeX, MyMouse.ViewSizeY, camera)
			...
			local torso = character
				...
				and character:FindFirstChild("Humanoid").Torso
				...
				if thisPather:IsValidPath() then
					...
					PathCompleteListener = thisPather.Finished:connect(function()
						...
						if finishPos and startPos and tick() - Utility.GetLastInput() > 2 then
							local exploreResults =
								ExploreWithRayCast(finishPos, ((startPos - finishPos) * XZ_VECTOR3).unit)
							-- Check for Nans etc..
							if
								exploreResults[1]
								and exploreResults[1]["Vector"]
								and exploreResults[1]["Vector"].magnitude >= 0.5
								and exploreResults[1]["Distance"] > 3
							then
								if CameraModule then
									local rotatedCFrame = CameraModule:LookAtPreserveHeight(
										finishPos + exploreResults[1]["Vector"] * exploreResults[1]["Distance"]
									)
									local finishedSignal, duration = CameraModule:TweenCameraLook(rotatedCFrame)
								end
							end
